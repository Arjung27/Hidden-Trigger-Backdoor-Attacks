...
class DenseNet(nn.Module):
    def __init__(self, block, nblocks, growth_rate=12, reduction=0.5, num_classes=10, train_dp=0, \
                test_dp=0, bdp=0, poison=False):
    ...
    # def forward2(self, x, pretrained, penu=False):
    #     feat = self.penultimate(x)
    #     if penu:
    #         return feat
    #     out = self.linear(feat)
...
def _densenet(block, nblocks, growth_rate=12, reduction=0.5, num_classes=10, train_dp=0, test_dp=0, bdp=0, \
                pretrained=True, progress=True, poison=False, **kwargs):

    model = DenseNet(Bottleneck, [6,12,24,16], growth_rate=32, train_dp=0, test_dp=0, bdp=0, poison=poison)

    if pretrained:
        model_file_name = '/nfshomes/arjgpt27/Experiments/Hidden-Trigger-Backdoor-Attacks_cifar/checkpoints/DenseNet121_seed_1000_normalize=True_augment=False_optimizer=SGD_epoch=199.t7'
...
def densenet121(pretrained=True, progress=True, poison=False, **kwargs):

    return _densenet(Bottleneck, [6,12,24,16], growth_rate=32, train_dp=0, \
        test_dp=0, bdp=0, pretrained=pretrained, progress=progress, poison=poison, **kwargs)
