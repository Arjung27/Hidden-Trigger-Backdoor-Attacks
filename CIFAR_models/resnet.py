...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, train_dp=0, test_dp=0, droplayer=0, bdp=0,
                                            middle_feat_num=1, poison=False):
...
def _resnet(block, nblocks, train_dp=0, test_dp=0, droplayer=0, bdp=0, pretrained=True, poison=False, **kwargs):

    model =  ResNet(block, nblocks, train_dp=train_dp, test_dp=test_dp, droplayer=droplayer, \
                    bdp=bdp, poison=poison)

    if pretrained:
        model_file_name = '/nfshomes/arjgpt27/Experiments/Hidden-Trigger-Backdoor-Attacks_cifar/checkpoints/resnet18_seed_1000_normalize=True_augment=False_optimizer=SGD_epoch=199.t7'
        model.load_state_dict(torch.load(model_file_name)['net'])
    
    return model 


def ResNet18(train_dp=0, test_dp=0, droplayer=0, bdp=0, pretrained=False, poison=False):
    return _resnet(BasicBlock, [2,2,2,2], train_dp=train_dp, test_dp=test_dp, droplayer=droplayer, bdp=bdp, \
                    pretrained=pretrained, poison=poison)
