...
save_dir	= options["save_dir"].format(data_name, model_name, model_name2, str(feature_size))
seed        = None
gpu         = int(options["gpu"])
...
logfile     = options["logfile"].format(data_name, model_name, model_name2, str(feature_size), experimentID, rand_loc, eps, patch_size, trigger_id)
...
def main_worker():
	...
	if data_name.upper() == 'CIFAR' and model_name == 'alexnet':
		# print('-------------------', feature_size)
		model = alexnet(pretrained=True, poison=True, feature_size=feature_size)

	elif data_name.upper() == 'CIFAR100' and model_name == 'alexnet':
		model = alexnet(pretrained=True, poison=True, feature_size=feature_size)
