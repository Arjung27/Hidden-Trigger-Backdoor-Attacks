...
feature_size = int(options["feature_size"])
poison_root	= options["poison_root"].format(data_name, model_name2, model_name, str(feature_size))
gpu         = int(options["gpu"])
...
logfile     = options["logfile"].format(data_name, model_name2, model_name, str(feature_size), experimentID, rand_loc, eps, patch_size, num_poison, trigger_id)
lr			= float(options["lr"])
...
feature_size = int(options["feature_size"])
save_dir	= options["save_dir"].format(data_name, model_name2, model_name, str(feature_size))
...
def initialize_model_cifar100(model_name, num_classes, feature_extract, use_pretrained=True):

	model_ft = None
	input_size = 0
	
	from CIFAR100_models.htbd_alexnet_cifar100 import alexnet, NormalizeByChannelMeanStd

	if model_name == "alexnet":
		""" Alexnet
		"""
		model_ft = alexnet(pretrained=use_pretrained, feature_size=feature_size)
		set_parameter_requires_grad(model_ft, feature_extract)
		num_ftrs = model_ft.classifier[6].in_features
		# num_ftrs1 = model_ft.classifier[4].in_features
		# model_ft.classifier[4] = nn.Linear(num_ftrs1,num_ftrs)
...
def initialize_model_cifar(model_name, num_classes, feature_extract, use_pretrained=True):
	...
	from CIFAR_models.alexnet import alexnet, NormalizeByChannelMeanStd
	...
	if model_name == "resnet":
		...
		"""
		# input()
		model_ft = alexnet(pretrained=True, feature_size=feature_size)
		set_parameter_requires_grad(model_ft, feature_extract)
		# num_ftrs = model_ft.classifier.in_features
		num_ftrs = model_ft.linear.in_features
		model_ft.linear = nn.Linear(num_ftrs, num_classes)
...
if data_name.upper() == 'CIFAR':
	...
	model_ft, input_size = initialize_model_cifar100(model_name, num_classes, feature_extract, use_pretrained=True)
...
with open("data/{}/{}/test_filelist.txt".format(data_name, experimentID), "w") as f1:
	...
	if num_classes == src_classes:
		...
		for i, wnid in enumerate(all_wnids):
			...
			if wnid==target_wnid:
				target_index=i
			with open("{}_data_list/test/".format(data_name) + wnid + ".txt".format(data_name), "r") as f2:
				lines = f2.readlines()
				for line in lines:
					f1.write(line.strip() + " " + str(i) + "\n")
...
if data_name.upper() == 'CIFAR':
	...
	print("CIFAR100 CIFAR100 CIFAR100 CIFAR100")
	normalize = NormalizeByChannelMeanStd(
